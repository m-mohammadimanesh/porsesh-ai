        ...
        <div className="markdown-body break-words w-full overflow-x-hidden">
          <ReactMarkdown
            ...
            components={{
              ...
              table: ({node, ...props}) => (
                <div className="my-4 w-full overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
              ...
              pre: ({node, ...props}) => (
                <pre className="bg-[#0d1117] dark:bg-slate-950 text-slate-50 p-4 rounded-xl my-4 overflow-x-auto text-sm border border-slate-800/80 shadow-md font-mono selection:bg-blue-500/30" {...props} />
              ),
              code: ({node, className, children, ...props}) => {
                ...
                return isInline ? (
                  <code className="bg-slate-100 dark:bg-slate-800 text-pink-600 dark:text-pink-400 px-1.5 py-0.5 rounded-md text-[0.9em] font-mono tracking-tight font-medium" {...props}>
                    {children}
                  </code>
                ) : (
                  <code className="font-mono text-[inherit] bg-transparent p-0" {...props}>
                    {children}
                  </code>
                );
              },
